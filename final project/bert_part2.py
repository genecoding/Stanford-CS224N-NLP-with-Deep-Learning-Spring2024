# Add mean pooling and with_embeds (input embeddings directly for SMART loss); 
# keep bert.py unchanged for part 1.
...
class BertSelfAttention(nn.Module):
  ...
  def attention(self, key, query, value, attention_mask):
    ...
    score = query @ key.transpose(-2, -1) / hs**0.5
    # in this mask, non-padding tokens are 0, padding tokens are -10000
    # score = score.masked_fill(attention_mask != 0, -1e-10)
    score += attention_mask  # to pass the sanity check
    weight = self.dropout(F.softmax(score, dim=-1))
    # (B, nh, T, T) x (B, nh, T, hs) -> (B, nh, T, hs)
    weighted_values = weight @ value
    weighted_values = weighted_values.transpose(1, 2).reshape(B, T, -1)  # (bs, seq_len, hidden_size)
...
class BertLayer(nn.Module):
  ...
  def forward(self, hidden_states, attention_mask):
    ...
    layer_output = self.add_norm(add_norm_output, 
                                 feed_forward_output, 
                                 self.out_dense, 
                                 self.out_dropout, 
                                 self.out_layer_norm)
...
class BertModel(BertPreTrainedModel):
  ...
  def embed(self, input_ids):
    ...
    ### TODO
    embed_output = input_embeds + pos_embeds + tk_type_embeds
    embed_output = self.embed_dropout(self.embed_layer_norm(embed_output))
  ...
  def forward(self, input_ids, attention_mask, with_embeds=False):
    """
    input_ids: [batch_size, seq_len], seq_len is the max length of the batch
    attention_mask: same size as input_ids, 1 represents non-padding tokens, 0 represents padding tokens
    with_embeds: if input embeddings directly, set it to True
    """
    # Get the embedding for each input token.
    # embedding_output = self.embed(input_ids=input_ids)
    embedding_output = input_ids if with_embeds else self.embed(input_ids=input_ids)
    ...
    sum_embeds = (sequence_output * mask).sum(dim=1)
    sum_mask = mask.sum(dim=1).clamp(min=1e-8)  # avoid division by 0
    mean_pooled = sum_embeds / sum_mask
    
    # return {'last_hidden_state': sequence_output, 'pooler_output': first_tk}
    return {'last_hidden_state': sequence_output, 'pooler_output': mean_pooled}

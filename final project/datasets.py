...
class SentencePairCatDataset(Dataset):
    """Concatenate sent1 and sent2 for cross encoding."""
    def __init__(self, dataset, args, isRegression=False):
        self.dataset = dataset
        self.p = args
        self.isRegression = isRegression 
...
# Unlike SentencePairCatDataset, we do not load labels in SentencePairCatTestDataset.
class SentencePairCatTestDataset(Dataset):
    """Concatenate sent1 and sent2 for cross encoding."""
    def __init__(self, dataset, args):
        self.dataset = dataset
        self.p = args
...
#     """Triplet dataset for MultipleNegativesRankingLoss with Hard Negatives."""
#     def __init__(self, dataset, args):
#         self.dataset = dataset
#         self.p = args
#         self.tokenizer = BertTokenizer.from_pretrained('bert-base-uncased')
...
#     def pad_data(self, data):
#         sent1, sent2, sent3 = map(list, zip(*data))  # anchor, positive, hard negative
    ...
    if split == 'test':
        ...
        with open(sentiment_filename, 'r') as fp:
            for record in csv.DictReader(fp,delimiter = '\t'):
                ...
                # to also use CFIMDB dataset to train sentiment analysis,
                # turn labels 0(negative) and 1(positive) to 0(negative) and 4(positive)
                # to align SST dataset
                # if 'cfimdb' in sentiment_filename:
                #     label *= 4
    ...
    if split == 'test':
        ...
        with open(paraphrase_filename, 'r') as fp:
            for record in csv.DictReader(fp,delimiter = '\t'):
                try:
                    # use triplet dataset
                    # if 'train-triplet' in paraphrase_filename:
                    #     paraphrase_data.append((preprocess_string(record['anchor']),
                    #                             preprocess_string(record['positive']),
                    #                             preprocess_string(record['hard_negative'])))
                    #     continue
                    
                    # only use datapoints of label=1
                    # if 'quora-train' in paraphrase_filename and float(record['is_duplicate']) == 0.0:
                    #     continue

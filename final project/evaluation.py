...
Add .to(torch.float32)/.float() to .cpu().numpy() to be compatible with AMP using bfloat16.
'''
...
# Evaluate multitask model on paraphrase detection only.
@torch.no_grad()
def model_eval_para(dataloader, model, device):
    ...
    for step, batch in enumerate(tqdm(dataloader, desc=f'eval', disable=TQDM_DISABLE)):
        ...
        y_hat = logits.sigmoid().round().flatten().cpu().float().numpy()
        b_labels = b_labels.flatten().cpu().float().numpy()
...
# Evaluate multitask model on semantic textual similarity only.
@torch.no_grad()
def model_eval_sts(dataloader, model, device):
    ...
    for step, batch in enumerate(tqdm(dataloader, desc=f'eval', disable=TQDM_DISABLE)):
        ...
        y_hat = logits.flatten().cpu().float().numpy()
        b_labels = b_labels.flatten().cpu().float().numpy()
...
def model_eval_multitask(sentiment_dataloader,
    ...
    with torch.no_grad():
        ...
        for step, batch in enumerate(tqdm(sentiment_dataloader, desc=f'eval', disable=TQDM_DISABLE)):
            ...
            logits = model.predict_sentiment(b_ids, b_mask)
            y_hat = logits.argmax(dim=-1).flatten().cpu().float().numpy()
            b_labels = b_labels.flatten().cpu().float().numpy()

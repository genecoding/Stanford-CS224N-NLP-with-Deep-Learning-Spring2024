...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        truncated = document[:truncated_len]
        
        mask_len_expected = truncated_len // 4
        # make the len of mask at least 1
        mask_len = random.randint(1, 2 * mask_len_expected - 1)  # mean = (a + b) / 2
        # make the len of prefix and suffix at least 1
        mask_start = random.randint(1, truncated_len - mask_len - 1)
        mask_end = mask_start + mask_len
        prefix = truncated[:mask_start]
        suffix = truncated[mask_end:]
        masked_content = truncated[mask_start:mask_end]

...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
        to_parse = unfinished_parses[:batch_size]  # the num of to_parse may < batch_size
        transitions = model.predict(to_parse)
        
        for i, parse in enumerate(to_parse):
            parse.parse_step(transitions[i])
            
            # if complete (empty buffer and stack of size 1), remove the parse

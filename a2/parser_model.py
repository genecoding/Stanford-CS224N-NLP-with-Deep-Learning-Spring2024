...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.embed_to_hidden_weight = nn.Parameter(torch.randn(n_features*self.embed_size, hidden_size, 
                                                               dtype=torch.float32))  # W
        self.embed_to_hidden_bias = nn.Parameter(torch.randn(hidden_size, dtype=torch.float32))  # b1
        self.hidden_to_logits_weight = nn.Parameter(torch.randn(hidden_size, n_classes, dtype=torch.float32))  # U
        self.hidden_to_logits_bias = nn.Parameter(torch.randn(n_classes, dtype=torch.float32))  # b2
    ...
    def embedding_lookup(self, w):
        ...
        x = self.embeddings[torch.flatten(w)].reshape(w.shape[0], -1)
        ### END YOUR CODE
        return x
    ...
    def forward(self, w):
        ...
        h = self.dropout(F.relu(x.matmul(self.embed_to_hidden_weight) + self.embed_to_hidden_bias))

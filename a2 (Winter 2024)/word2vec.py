...
def naiveSoftmaxLossAndGradient(
    ...
    ### to integer overflow. 
    y_hat = softmax(np.dot(outsideVectors, centerWordVec))  # (num_vocabs,)
...
def negSamplingLossAndGradient(
    ...
    sigmoid_uovc = sigmoid(np.dot(outsideVectors[outsideWordIdx], centerWordVec))
    sigmoid_neg_uwsvc = sigmoid(-1 * np.dot(outsideVectors[negSampleWordIndices], centerWordVec))  # (num_negSamples,)
    
    loss = -np.log(sigmoid_uovc) - np.sum(np.log(sigmoid_neg_uwsvc))
    gradCenterVec = np.dot(sigmoid_uovc - 1, outsideVectors[outsideWordIdx])\
                    + np.sum((1 - sigmoid_neg_uwsvc)[:, None] * outsideVectors[negSampleWordIndices], axis=0)
    # when w == o
    gradOutsideVecs[outsideWordIdx] = (sigmoid_uovc - 1) * centerWordVec
    # when w != o. since the same word may be sampled multiple times, use np.add.at to accumulate
    np.add.at(gradOutsideVecs, negSampleWordIndices, (1 - sigmoid_neg_uwsvc)[:, None] * centerWordVec)
    ### END YOUR CODE
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for word in outsideWords:
        ...
        results = word2vecLossAndGradient(centerWordVec, outsideWordIdx, outsideVectors, dataset)
        loss += results[0]
        gradCenterVecs[word2Ind[currentCenterWord]] += results[1]

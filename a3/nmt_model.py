...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.att_projection = nn.Linear(hidden_size*2, hidden_size, bias=False)  # W_{attProj}
        self.combined_output_projection = nn.Linear(hidden_size*3, hidden_size, bias=False)  # W_{u}
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        X = self.model_embeddings.source(source_padded).permute(1, 2, 0)  # (src_len, b, e) -> (b, e, src_len)
        X = self.post_embed_cnn(X).permute(2, 0, 1)  # (b, e, src_len) -> (src_len, b, e)
        packed_X = pack_padded_sequence(X, source_lengths)
        packed_enc_hiddens, (last_hidden, last_cell) = self.encoder(packed_X)  # last_hidden, last_cell: (2, b, h)
        enc_hiddens, _ = pad_packed_sequence(packed_enc_hiddens)  # (src_len, b, h*2)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for Y_t in Y:  # Y_t: (b, e)
            ...
            dec_state, o_t, _ = self.step(Ybar_t, dec_state, enc_hiddens, enc_hiddens_proj, enc_masks)  # o_t: (b, h)
